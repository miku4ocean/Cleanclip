...
class AntiCrawlBypass {
  ...
  async bypassAntiCrawl() {
    ...
    if (this.isSocialMediaPlatform()) {
      console.log('Detected social media platform, using specialized approach...');
      return await this.handleSocialMediaPlatform();
    }
  ...
  /**
   * Specialized handling for social media platforms
   */
  async handleSocialMediaPlatform() {
    const hostname = window.location.hostname.toLowerCase();
    
    if (hostname.includes('facebook.com')) {
      return await this.handleFacebook();
    } else if (hostname.includes('twitter.com')) {
      return await this.handleTwitter();
    } else if (hostname.includes('instagram.com')) {
      return await this.handleInstagram();
    } else if (hostname.includes('linkedin.com')) {
      return await this.handleLinkedIn();
    }
  ...
  async handleFacebook() {
    console.log('Handling Facebook content...');
    
    // Wait longer for Facebook's heavy JavaScript
    await this.waitForContent(10000);
    
    // Try multiple strategies for Facebook
    let content = null;
    
    // Strategy 1: Try to find post content
    const postSelectors = [
    ...
    for (const selector of postSelectors) {
      ...
      if (element && this.isVisible(element)) {
        ...
        if (text && text.length > 20) {
          content = {
            title: this.extractFacebookTitle(),
            text: text,
    ...
    // Strategy 2: Try Open Graph data
    if (!content) {
      content = this.extractFromMeta();
      if (content) {
        content.source = 'facebook_meta';
      }
    }
    
    // Strategy 3: Show helpful message for login-required content
    if (!content) {
      return {
        title: 'Facebook 內容',
        text: 'Facebook 內容需要登入才能查看。請手動複製貼上內容到 CleanClip 進行處理。',
        source: 'facebook_notice'
      };
    }
  ...
  /**
   * Extract Facebook post title
   */
  extractFacebookTitle() {
    // Try to find author name or page title
   ...
   */
  async handleTwitter() {
    console.log('Handling Twitter content...');
    
    await this.waitForContent(8000);
    ...
    return {
      title: 'Twitter 內容',
      text: 'Twitter 內容可能需要登入或已被設為私人。請手動複製內容。',
      source: 'twitter_notice'
    };
  }
  ...
  async handleInstagram() {
    return {
      title: 'Instagram 內容',
      text: 'Instagram 內容通常需要登入才能查看。建議使用手動模式複製內容。',
      source: 'instagram_notice'
    };
  }
  ...
  async handleLinkedIn() {
    return {
      title: 'LinkedIn 內容',
      text: 'LinkedIn 內容可能需要登入才能完整查看。建議切換到手動模式處理內容。',
      source: 'linkedin_notice'
    };
  }
}
